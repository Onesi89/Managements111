...
public class Review {
			...
			MemberStart.First();	
				...
				try {
					reader = new BufferedReader
							(new FileReader("C:\\DDGCinema-master\\DDGCinema-master\\[02] 데이터 파일\\리뷰모음.txt"));
					String line = null;
				...
				try {
					BufferedWriter writer = new BufferedWriter
							(new FileWriter("C:\\DDGCinema-master\\DDGCinema-master\\[02] 데이터 파일\\리뷰모음.txt", true));
	...
	private void reviewModify() {
		...
		//변경할 데이터 입력 또는 메뉴 선택 		
		String reviewModifyInput = sc.nextLine();
		...
		if(reviewModifyInput.equals("1")) {//뒤로가기
			...
			try {
				...
				editReview(reviewModifyInput); //[선택한 영화] 리뷰 수정
				
			} catch (Exception e) {
				System.out.println("Review.reviewModify()");
				e.printStackTrace();
			}
	...
	//[선택한 영화] 리뷰 수정
	private void editReview(String reviewModifyInput) {
	...
	private void reviewGradeModify(String reviewModifyInput) {
		...
		if(input.equals("B")) {//뒤로가기
			...
			if(Integer.parseInt(input) >= 1 && Integer.parseInt(input) <=5) {
				...
				//현재 고객정보 목록 불러오기
				String path = "C:\\DDGCinema-master\\DDGCinema-master\\[02] 데이터 파일\\리뷰모음.txt";
				File dir = new File(path);
